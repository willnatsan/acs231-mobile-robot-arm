...
float s3AngleMax;
int s3Width;
float coordinatesInitial[] = {17.2, 5.7, 0.0}; // {x, y, z}
float coordinatesFinal[] = {17.2, -4.3, 0.0}; // {x, y, z}
float xCurr;
float yCurr;
float zCurr;
...
unsigned long controlRate = 1000; // milliseconds
...
void loop() {
  ...
  while (yCurr >= coordinatesFinal[1]){
    ...
    if (elapsedTime >= controlRate) {
      ...
      s1Width = moveServo(s1, 1503, s1Angle);
